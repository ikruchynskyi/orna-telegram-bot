...
def build_text(progress=None) -> str:
    """Fetch every sheet in _TABLES and return the whole corpus as the text
    orna_knowledge.txt holds - WITHOUT writing anything.

    Split out from main() so orna_knowledge.py can refresh itself on a TTL
    (see its _load) using the same builder the script uses, rather than a
    second copy of the format that could drift from the parser reading it.
    One HTTP request per tab, so ~16 on a cold build; the caller decides
    whether that belongs on a background thread."""
    sections = []
    for title, spreadsheet_id, gid, source in _TABLES:
        ...
        if progress:
            progress(f"{title}: {len(lines)} lines")
    return _HEADER + "\n\n".join(sections) + "\n"


def main() -> None:
    text = build_text(progress=print)
